...
module.exports = async function handler(req, res) {
  ...
  const mime = match[1]; const base64 = match[2];
  ...
  try {
    const blob = await put(pathname, Buffer.from(base64, 'base64'), { access: 'public', contentType: mime, addRandomSuffix: false, allowOverwrite: true });
    return res.status(200).json({ ok: true, storage: 'vercel-blob', image: { src: blob.url, caption: caption || '', createdAt: new Date().toISOString(), clientUploadId: safeUploadId, contentHash: contentHash || '' } });
  } catch (error) {
    return res.status(500).json({ error: 'Vercel Blob upload failed', detail: error && error.message ? error.message : String(error) });
  }
};
